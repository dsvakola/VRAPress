...
if (is_post()) {
    ...
    $slug = unique_slug('pages', $slugInput !== '' ? $slugInput : $title, $id);
    $contentValue = $_POST['content'] ?? '';
    $status = $_POST['status'] ?? 'draft';
    $meta_title = trim($_POST['meta_title'] ?? '');
    $meta_description = trim($_POST['meta_description'] ?? '');

    $update = db()->prepare('UPDATE pages SET title = ?, slug = ?, content = ?, status = ?, meta_title = ?, meta_description = ?, updated_at = NOW() WHERE id = ?');
    $update->execute([$title, $slug, $contentValue, $status, $meta_title, $meta_description, $id]);
...
<div class="card">
    <div class="toolbar">
        <h2>Edit Page</h2>
        <a class="btn light" href="<?= public_page_url($page['slug']) ?>" target="_blank">View Page</a>
    </div>
    <form method="post">
